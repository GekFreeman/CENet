...
# root_path ="/userhome/root/share/Original_images/" # 31
root_path = "/userhome/root/workspace/zhangc/datasets/CXR/" # 2
# root_path ="/userhome/research/continualLearning/deep-transfer-learning/MUDA/MFSAN/dataset/office_caltech_10/" # 10
# root_path ="/userhome/research/continualLearning/deep-transfer-learning/MUDA/MFSAN/dataset/office31_raw_image/Original_images/" # 31
        ...
        #for inner_flag in range(1,7):
            ...
            for original_source_name in sources:
                ...
                for epoch in range(start_epoch, config['meta_epochs']):
                    ...
                    trn_tgt_pair3_sim_loader1 = data_loader.load_training_index(root_path, target_name, batch_size//4, tgt_idx, kwargs)
                    trn_src_pair3_sim_loader2 = data_loader.load_training_index(root_path, source_name, batch_size //4, src_indices, kwargs)

                    src_qry_idx1, _, src_qry_idx2, _ = index_generate_diff(src_indices, src_qry_label[src_indices], shuffle=False)
                    trn_src_pair4_diff_loader = data_loader.load_training_index(root_path, source_name, batch_size//4, src_qry_idx2, kwargs)
                    ...
                    writer.add_scalar(f"Acc/target_hard_{original_target_name}_{original_source_name}_{exp_name}", tgt_hard_acc, epoch)
...
#                         writer.add_scalar(f"loss/finetune_cls_loss_{original_source_name}_{exp_name}", cls_loss.item(), iters)
#                         writer.add_scalar(f"loss/finetune_mmd_loss_{original_source_name}_{exp_name}", mmd_loss.item(), iters)
#                         writer.add_scalar(f"loss/finetune_pull_loss_{original_source_name}_{exp_name}", pull_loss.item(), iters)
#                         writer.add_scalar(f"loss/finetune_push_loss_{original_source_name}_{exp_name}", push_loss.item(), iters)
...
def split_target(model, data_loader, source_name, loader_type=1, idx_init=0, k=10, index=None, last_simple_idx=None, last_simple_label=None):
    """
    loader_type: 1表示源域，0表示目标域
    last_simple_idx: 上一个epoch的累积简单样本;
    last_simple_label: 上一个epoch的累积简单样本标注;
    """
    test_loss = 0
    correct = 0
    correct1 = 0
...
#         else:
#             # 替换为新的标签
#             tmp_idx = np.where(last_simple_idx == x)[0]
#             last_simple_label[tmp_idx.item()] = simple_label[i]
